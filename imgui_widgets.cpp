...
bool ImGui::Keybind(const char* label, int* key, int* mode, bool show_label)
{
    ...
    const ImRect rect(window->DC.CursorPos + ImVec2(0, 0), window->DC.CursorPos + ImVec2(width, 17.f));
    ...
    it_anim->second.text = ImLerp(it_anim->second.text, g.ActiveId == id ? c::accent_color : hovered ? c::text::text_hov : c::text::text, ImGui::GetIO().DeltaTime * 6.f);

    it_anim->second.slow_show = ImLerp(it_anim->second.slow_show, clickable.Min.x - rect.Min.x, g.IO.DeltaTime * 15.f);

    window->DrawList->AddRectFilled(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y), clickable.Max - ImVec2(0, 0), GetColorU32(c::keybind::background), c::keybind::rounding);
    window->DrawList->AddRect(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y), clickable.Max - ImVec2(0, 0), GetColorU32(c::keybind::border), c::keybind::rounding);

    if (show_label) window->DrawList->AddText(rect.Min + ImVec2(25, -2), GetColorU32(it_anim->second.text), label);

    PushStyleColor(ImGuiCol_Text, c::text::text_active);
    RenderTextClipped(ImVec2(it_anim->second.slow_show + rect.Min.x, clickable.Min.y - 2), clickable.Max, buf_display, NULL, &label_size, ImVec2(0.5f, 0.5f));
    PopStyleColor();
    ...
    PushStyleVar(ImGuiStyleVar_WindowRounding, c::keybind::rounding);
    PushStyleColor(ImGuiCol_WindowBg, c::keybind::border);
    if (it_anim->second.alpha >= 0.01f)
    {
        SetNextWindowPos(ImVec2(clickable.GetCenter().x - 34, clickable.Max.y + 5));
        SetNextWindowSize(ImVec2(69, 75));
...
struct tab_state
{
    ImVec4 background, image, text, gradient;
};
...
bool ImGui::Tabs(bool selected, ImTextureID image, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::tabs::border : c::tabs::border_invisible, g.IO.DeltaTime * 6.f);
    it_anim->second.gradient = ImLerp(it_anim->second.gradient, selected ? c::accent_color : c::tabs::border_invisible, g.IO.DeltaTime * 6.f);

    it_anim->second.image = ImLerp(it_anim->second.image, selected ? c::accent_color :  hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min + ImVec2(size_arg.x - 30, 0), bb.Max, GetColorU32(it_anim->second.background), GetColorU32(it_anim->second.gradient), GetColorU32(it_anim->second.gradient), GetColorU32(it_anim->second.background), c::tabs::rounding);
    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(2, 2), bb.Max - ImVec2(2, 2), GetColorU32(it_anim->second.background), c::tabs::rounding);

    GetWindowDrawList()->AddCircleFilled(bb.Max -ImVec2(15, size_arg.y / 2), 6.f, GetColorU32(it_anim->second.gradient), 100.f);
    ...
    RenderTextClipped(bb.Min + ImVec2(34, -1), bb.Max, label, NULL, &label_size, ImVec2(0.0, 0.5), &bb);
    PopStyleColor();
...
bool ImGui::Subtab(bool selectable, const char* label, const ImVec2& size_arg)
{
    ...
    it_anim->second.line = ImLerp(it_anim->second.line, selectable ? c::accent_color : c::tabs::border_invisible, g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, selectable ? c::accent_color : hovered ? c::text::text_hov : c::text::text, g.IO.DeltaTime * 6.f);

    it_anim->second.text_offset = ImClamp(it_anim->second.text_offset + (6.f * g.IO.DeltaTime * (selectable ? 10.f : -5.f)), 0.f, 6.f);

    ImGui::GetWindowDrawList()->AddShadowRect(bb.Min + ImVec2(size_arg.x / 4, size_arg.y - 3), bb.Max - ImVec2(size_arg.x / 4, 2), GetColorU32(it_anim->second.line), 20.f, ImVec2(0, 0), 30.f, ImDrawCornerFlags_Top);
    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(size_arg.x / 4, size_arg.y - 3), bb.Max - ImVec2(size_arg.x / 4, 0), GetColorU32(it_anim->second.line), 30.f, ImDrawCornerFlags_Top);
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::accent_color : c::button::background_invisible, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(c::button::gradient_color0), GetColorU32(c::button::gradient_color0), GetColorU32(c::button::gradient_color1), GetColorU32(c::button::gradient_color1), c::button::rounding);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(c::button::border), c::button::rounding);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRectFilled(grab_rect.Min - ImVec2(6, 0), grab_rect.Max - ImVec2(6, 0), GetColorU32(c::scrollbar::background), c::scrollbar::rounding);
    window->DrawList->AddRect(grab_rect.Min - ImVec2(6, 0), grab_rect.Max - ImVec2(6, 0), GetColorU32(c::scrollbar::border), c::scrollbar::rounding);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, *v ? c::accent_color : c::checkbox::background_invisible, g.IO.DeltaTime * 6.f);
    it_anim->second.mark = ImLerp(it_anim->second.mark, *v ? c::checkbox::mark_active : c::checkbox::mark_inactive, g.IO.DeltaTime * 6.f);
    ...
    it_anim->second.clip_mark = ImLerp(it_anim->second.clip_mark, *v ? square_sz : 0.f, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));

    GetWindowDrawList()->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, GetColorU32(c::checkbox::gradient_color0), GetColorU32(c::checkbox::gradient_color0), GetColorU32(c::checkbox::gradient_color1), GetColorU32(c::checkbox::gradient_color1), c::checkbox::rounding);
    GetWindowDrawList()->AddRect(check_bb.Min, check_bb.Max, GetColorU32(c::checkbox::border), c::checkbox::rounding);
    GetWindowDrawList()->AddRectFilled(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background), c::checkbox::rounding);

    ImGui::PushClipRect(check_bb.Min, check_bb.Min + ImVec2(it_anim->second.clip_mark, square_sz), true);
    RenderCheckMark(GetWindowDrawList(), check_bb.Min + ImVec2(square_sz / 2, square_sz / 2) / 2, GetColorU32(it_anim->second.mark), (square_sz / 2));
    ImGui::PopClipRect();
...
{
    std::string name_label = "picker", name_picker = label + name_label;

    ImGui::Checkbox(label, v);

    ImGui::SetCursorPosY(GetCursorPosY() - (GetStyle().ItemSpacing.y + 21));

    ImGui::ColorEdit4(name_picker.c_str(), color, flags | ImGuiColorEditFlags_NoLabel);

}

void ImGui::Keybindbox(const char* label, bool* v, int* key, int* m)
{
    std::string name_label = "keybind", name_keybind = label + name_label;

    ImGui::Checkbox(label, v);

    ImGui::SetCursorPosY(GetCursorPosY() - (GetStyle().ItemSpacing.y + 18));

    ImGui::Keybind(name_keybind.c_str(), key, m, false);

}

void ImGui::KeybindPickerbox(const char* label, bool* v, int* key, int* m, float color[3], ImGuiColorEditFlags flags)
{
    std::string name_label0 = "keybind", name_keybind = label + name_label0;
    std::string name_label1 = "picker", name_picker = label + name_label1;

    ImGui::Checkbox(label, v);

    ImGui::SetCursorPos(GetCursorPos() - ImVec2(25, (GetStyle().ItemSpacing.y + 18)));
...
{
    ImVec4 background, text;
    float open, alpha, combo_size = 0.f;
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    const ImRect bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, 25));
    ...
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 27) + 3 : 0.f, g.IO.DeltaTime * 12.f);
    ...
    GetWindowDrawList()->AddRectFilled(bb.Min, ImVec2(value_x2 + GetFrameHeight(), bb.Max.y), GetColorU32(c::combo::background), c::combo::rounding);
    GetWindowDrawList()->AddRect(bb.Min, ImVec2(value_x2 + GetFrameHeight(), bb.Max.y), GetColorU32(c::combo::border), c::combo::rounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));

    PushFont(font::icon_font);
    RenderTextClipped(bb.Min + ImVec2(w - 26, 0), ImVec2(value_x2 + GetFrameHeight(), bb.Max.y - 2), "h", NULL, NULL, ImVec2(0.5, 0.5), NULL);
    PopFont();

    RenderTextClipped(bb.Min + ImVec2(5, 0), ImVec2(value_x2, bb.Max.y - 1), preview_value, NULL, NULL, ImVec2(0.0f, 0.5f), NULL);
    PopStyleColor();
    ...
    ImGui::SetNextWindowPos(ImVec2(bb.Min.x + 2, bb.Max.y + pos));
    ...
    PushStyleColor(ImGuiCol_Border, c::combo::border);
...
void ImGui::MultiCombo(const char* label, bool variable[], const char* labels[], int count)
{
    ...
    if (ImGui::BeginCombo(label, preview.c_str(), count, 0, true)) // draw start
    {
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags, ImRect* out_grab_bb)
{
    ...
    const float slider_usable_pos_min = bb.Min[axis] + grab_padding + grab_sz * 0.1f;
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(26, 24), window->DC.CursorPos + ImVec2(w, 29));
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x, 0.0f));

    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ItemSize(ImRect(total_bb.Min, total_bb.Max + ImVec2(0, 28)));
    if (!ItemAdd(total_bb, id, &frame_bb, temp_input_allowed ? ImGuiItemFlags_Inputable : 0))
        return false;
    ...
    GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::slider::background), c::slider::rounding);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        GetWindowDrawList()->AddRectFilled(frame_bb.Min + ImVec2(0, 0), ImVec2(it_anim->second.slow + frame_bb.Min.x + 5, grab_bb.Max.y + 1), GetColorU32(c::accent_color), c::slider::rounding);
        GetWindowDrawList()->AddCircleFilled(ImVec2(it_anim->second.slow + frame_bb.Min.x + 8, grab_bb.Max.y - 1), 7.f, GetColorU32(c::slider::circle), 100.f);
        GetWindowDrawList()->AddCircle(ImVec2(it_anim->second.slow + frame_bb.Min.x + 8, grab_bb.Max.y - 1), 7.f, GetColorU32(c::slider::background), 100.f, 1.f);
    }

    char value_buf[64];
    const char* value_buf_end = value_buf + DataTypeFormatString(value_buf, IM_ARRAYSIZE(value_buf), data_type, p_data, format);

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Max.x - w + 25, frame_bb.Min.y - 25), GetColorU32(it_anim->second.text), label);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(frame_bb.Min - ImVec2(0, 36), frame_bb.Max, value_buf, value_buf_end, NULL, ImVec2(1.0f, 0.5f));
    PopStyleColor();
...
{
    ImVec4 hint_color;
    float slow, alpha_line, hint_offset;
};
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, GetContentRegionMax().x - style.WindowPadding.x, (is_multiline ? g.FontSize * 8.0f : 25.f)); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + label_size.x, frame_size.y);
    ...
    if (!is_multiline)
    {
        ...
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(c::input::border), c::input::rounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x, frame_bb.Min.y, frame_bb.Min.x + inner_size.x, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(5, (size_arg.y - CalcTextSize(hint).y) / 2 - 1);
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            if (cursor_screen_rect.Overlaps(clip_rect)) {
                ...
                it_anim->second.alpha_line = ImLerp(it_anim->second.alpha_line, cursor_is_visible ? 1.f : 0.f, g.IO.DeltaTime * 6.f);

                PushStyleVar(ImGuiStyleVar_Alpha, it_anim->second.alpha_line);
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 3), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 3), GetColorU32(c::text::text_active));
                PopStyleVar();
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_Border, GetColorU32(c::picker::border));
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                if (ImGui::Button("Copy", ImVec2(100, 25)));
                ImGui::SameLine();
                if (ImGui::Button("Paste", ImVec2(100, 25)));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(25, 0), GetColorU32(c::text::text), label);
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const ImRect clickable(pos + ImVec2(width - 17, 2), pos + ImVec2(width, 19));
    ItemSize(ImRect(rect.Min, rect.Max + ImVec2(0, 2)));
    if (!ItemAdd(rect, id))
        return false;
    ...
    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min, clickable.Max + ImVec2(0, 0), GetColorU32(col_rgb), 3, ImVec2(0.f, 0.f), c::picker::rounding);
